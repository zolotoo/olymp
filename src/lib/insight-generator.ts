...
ОТВЕТ — строго JSON без markdown:
{
  ...
  "draft_message": "готовый текст DM на 'ты', 2–4 коротких предложения, тёплый человечный тон без воды и без markdown. Обращение по имени. Можно использовать эмодзи в меру. НЕ упоминай 'я бот', не давай ссылок (админ добавит сам). Конкретно про то, что цепляет именно его."
}`
}
